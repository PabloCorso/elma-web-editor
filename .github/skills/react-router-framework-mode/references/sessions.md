...
description:
  Cookie-based session management, authentication patterns, protected routes
...
React Router provides built-in utilities for cookie-based session management.
**Always use these instead of hand-rolling cookie implementations.**
...
- **`destroySession(session)`** - Generate `Set-Cookie` header that clears the
  session
...
**Important:** Always perform logout in an `action`, never in a `loader`. This
prevents [CSRF attacks][csrf].

## Authentication Middleware

Use middleware to set user context from session data, making it available to all
child routes.
...
Flash data is automatically cleared after being read—useful for one-time
messages:
...
**Gotcha:** With nested routes, multiple loaders run in parallel. If you use
flash data, ensure only one loader reads it to avoid race conditions.
...
Add new secrets to the front of the array. Old cookies are still readable, new
cookies use the first secret:
