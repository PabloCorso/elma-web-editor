...
description:
  Auto-generated types, typegen setup, typing loaders/actions/fetchers
tags:
  [
    types,
    typescript,
...
React Router generates route-specific types that provide type inference for URL
params, loader data, action data, and more.
...
Import the `Route` namespace from the `+types` directory relative to your route
file:
...
When using `useFetcher` to call an action from another route, type it with the
action's type:
