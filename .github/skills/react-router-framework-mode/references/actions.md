...
Actions handle data mutations (create, update, delete). After an action
completes, all loaders on the page automatically revalidate.
...
**Default to `useFetcher` for mutations** - it provides smoother UX with
optimistic updates and no page reload.
...
**Use `useFetcher` as the default for mutations** - it provides independent
state, no page navigation, and enables optimistic UI.
...
Use `fetcher.formData` to show expected results immediately. See
[pending-ui.md](./pending-ui.md#optimistic-ui-with-usefetcher-recommended-pattern)
