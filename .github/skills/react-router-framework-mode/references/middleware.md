...
See
https://reactrouter.com/how-to/middleware#changes-to-getloadcontextapploadcontext
for migration instructions.

## Overview

Middleware runs code before and after response generation. Executes in a nested
chain: parent → child on the way down, child → parent on the way up.
...
- Call `next()` only once
- If you don't need post-processing, skip calling `next()` (called
  automatically)
...
**Server middleware** (`middleware`) runs on document requests and `.data`
fetches:
...
Errors bubble to the nearest `ErrorBoundary`. The `next()` function always
