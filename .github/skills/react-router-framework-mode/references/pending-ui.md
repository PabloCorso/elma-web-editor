...
**Key principle:** Show expected results immediately using `fetcher.formData`,
then let React Router sync with the server.
...
For mutation patterns (when to use Form vs useFetcher), see
[actions.md](./actions.md#choosing-the-right-pattern).
...
**This is the standard pattern for mutations.** Show the expected result
immediately using `fetcher.formData`:
...
- **Automatic rollback** - if the action fails, loaders revalidate and reset to
  server state
...
- [special-files.md](./special-files.md#customizing-roottsx-complete-example) -
  Global loading indicators in root.tsx
