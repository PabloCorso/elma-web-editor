...
export class CameraUtils {
  static updateCamera(deltaX: number, deltaY: number, panSpeed: number = 1.0) {
  ...
  static updateZoom(
    ...
    if (clampedZoom !== currentZoom) {
      ...
      if (mouseX !== undefined && mouseY !== undefined) {
        this.adjustViewportForZoom(mouseX, mouseY, currentZoom, clampedZoom);
      }
    }
  }

  private static adjustViewportForZoom(
    mouseX: number,
    mouseY: number,
    oldZoom: number,
    newZoom: number
